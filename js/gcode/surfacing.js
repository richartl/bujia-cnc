(function () {
  ...
  function generateSurfacingAdaptiveGcode(parameters, adaptiveRows) {
    const normalized = Object.assign({}, parameters, {
      zPasses: adaptiveRows.length,
    });
    const warnings = validateSurfacingParameters(normalized);
    ...
    lines.push("( Pasadas Z: " + normalized.zPasses + ", profundidad total: Z-" + clean(normalized.totalZ) + " mm )");
    lines.push("( Step X: " + clean(normalized.stepX) + " mm, Feed XY: F" + clean(normalized.feedRate) + " )");
    lines.push("( Modo: Adaptive Passes )");
    lines.push("");
    ...
    adaptiveRows.forEach(function (row, index) {
      ...
      const feedRate = row.feedrate || normalized.feedRate;
      const rpm = row.rpm || normalized.spindle;

      lines.push("( ============================== )");
      lines.push("( PASADA Z " + (index + 1) + " DE " + adaptiveRows.length + " - Z-" + clean(depth) + " )");
      lines.push("( ============================== )");
      lines.push("M3 S" + clean(rpm));
      lines.push("G90");
      lines.push("G0 X0 Y0");
      lines.push("G1 Z-" + clean(depth) + " F" + clean(normalized.plungeRate));
      lines.push("G91");
      lines.push("F" + clean(feedRate));
      lines.push("");

      let coveredX = 0;
      let pass = 1;
      let dirY = normalized.startDirection === "positive" ? normalized.distY : -normalized.distY;

      while (coveredX < normalized.distX) {
        const remainingX = normalized.distX - coveredX;
        const moveX = Math.min(normalized.stepX, remainingX);

        lines.push("( Z" + (index + 1) + " / PASADA XY " + pass + " )");
        lines.push("G1 Y" + clean(dirY));
        if (moveX > 0) lines.push("G1 X" + clean(moveX));
        lines.push("");
  ...
  window.BujiaSurfacingGcode = {
    ...
    generateSurfacingAdaptiveGcode: generateSurfacingAdaptiveGcode,
  };
}());
